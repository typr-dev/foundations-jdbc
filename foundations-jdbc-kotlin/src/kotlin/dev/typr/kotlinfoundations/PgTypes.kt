...
open class PgTypes {
    ...
    open fun bpchar(length: Int): PgType<String> = JavaPgTypes.bpchar(length)

    open fun bpcharArray(n: Int): PgType<Array<String>> = JavaPgTypes.bpcharArray(n)

    open fun <E : Enum<E>> ofEnum(enumTypeName: String, fromString: (String) -> E): PgType<E> =
        JavaPgTypes.ofEnum(enumTypeName) { fromString(it) }
