...
open class OracleTypes {
    ...
    open val number: OracleType<java.math.BigDecimal> = OracleType(JavaOracleTypes.number)
    open val integer: OracleType<java.math.BigDecimal> = OracleType(JavaOracleTypes.integer)
    open val smallint: OracleType<java.math.BigDecimal> = OracleType(JavaOracleTypes.smallint)
    open val varchar2: OracleType<String> = OracleType(JavaOracleTypes.varchar2)
    open val char_: OracleType<String> = OracleType(JavaOracleTypes.char_)

    /** Alias for [char_] — aesthetic, avoids the Java-keyword `_` suffix. */
    open val character: OracleType<String> = char_
    ...
    open fun numberOf(precision: Int, scale: Int): OracleType<java.math.BigDecimal> = OracleType(JavaOracleTypes.numberOf(precision, scale))
    ...
    open fun <Row : Any> jsonArrayEncoded(parser: RowCodec<Row>): OracleType<Row> =
    ...
    open fun <Row : Any> jsonArrayEncodedList(parser: RowCodec<Row>): OracleType<List<Row>> =
        OracleType(JavaOracleTypes.jsonArrayEncodedList(parser.underlying))

    /** A JSON column type that stores a single row as a keyed JSON object: {"col": val, ...}. */
    open fun <Row : Any> jsonObjectEncoded(parser: RowCodecNamed<Row>): OracleType<Row> =
    ...
    open fun <Row : Any> jsonObjectEncodedList(parser: RowCodecNamed<Row>): OracleType<List<Row>> =
        OracleType(JavaOracleTypes.jsonObjectEncodedList(parser.underlying))

    /** Build a named Oracle OBJECT type from a RowCodecNamed. */
    open fun <Row : Any> compositeOf(objectTypeName: String, codec: RowCodecNamed<Row>): OracleType<Row> =
