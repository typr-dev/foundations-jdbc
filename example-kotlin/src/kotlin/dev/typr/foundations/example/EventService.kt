...
class EventService(private val tx: Transactor) {
    ...
    fun findEvent(id: EventId): Event? = EventRepo.eventById(id).transact(tx)
    ...
    fun createVenueWithEvent(
        ...
        val created = VenueRepo.createVenue(venue).run(mc)
    ...
    fun publishEvent(eventId: EventId): Event = tx.transact { mc ->
        val event = EventRepo.eventById(eventId).run(mc)
        ...
        EventRepo.updateEventStatus(EventStatus.PUBLISHED, eventId).run(mc)
        EventRepo.eventById(eventId).run(mc)!!
    }
    ...
    fun purchaseTickets(
        ...
        val venue = VenueRepo.venueById(event.venueId).run(mc)!!
        val currentCount = TicketRepo.countTicketsByEvent(eventId).run(mc)
        ...
        if (totalAfter.toInt() == venue.capacity) {
            EventRepo.updateEventStatus(EventStatus.SOLD_OUT, eventId).run(mc)
        }
    ...
    fun rateEvent(eventId: EventId, rating: Double) {
        ...
        EventRepo.addEventRating(rating, eventId).transact(tx)
    }
