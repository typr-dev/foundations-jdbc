...
class SqlServerTypes {
  ...
  def decimal(precision: Int, scale: Int): SqlServerType[BigDecimal] =
    JavaSqlServerTypes.decimal(precision, scale).bimap(jbd => BigDecimal(jbd), sbd => sbd.bigDecimal)

  def numeric(precision: Int, scale: Int): SqlServerType[BigDecimal] =
    JavaSqlServerTypes.numeric(precision, scale).bimap(jbd => BigDecimal(jbd), sbd => sbd.bigDecimal)
