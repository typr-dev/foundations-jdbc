...
public class StreamingQueryTest {
  ...
  private void createAndPopulate(Connection conn, String table, int count) throws SQLException {
    ...
    for (int i = 1; i <= count; i++) {
      conn.createStatement().execute(
          "INSERT INTO " + table + " VALUES (" + i + ", 'item" + i + "')");
    }
  }
  ...
  public void testBasicCursorIteration() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t1 ORDER BY id")
          .streamingQuery(itemCodec, 100);
  ...
  public void testToList() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t2 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
  ...
  public void testForEach() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t3 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
  ...
  public void testEnhancedForLoop() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t4 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
      List<Item> items = new ArrayList<>();
      for (var item : cursor) {
  ...
  public void testEmptyResultSet() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t5 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
  ...
  public void testNoSuchElementOnExhaustedCursor() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t6 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
      cursor.next();
      assertFalse(cursor.hasNext());
  ...
  public void testMapWithToList() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t7 ORDER BY id")
          .streamingQuery(itemCodec, 100);
  ...
  public void testMapWithProcessing() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t8 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      long count = streaming.map(cursor -> {
        long n = 0;
        for (var item : cursor) {
          n++;
        }
  ...
  public void testCombineTwoStreamingOperations() {
    withConnection(conn -> {
      ...
      conn.createStatement().execute(
          "CREATE TABLE t9b (id INTEGER, name VARCHAR)");
      conn.createStatement().execute(
          "INSERT INTO t9b VALUES (10, 'other1')");

      var s1 = Fragment.of("SELECT id, name FROM t9a ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var s2 = Fragment.of("SELECT id, name FROM t9b ORDER BY id")
          .streamingQuery(itemCodec, 100);

      var combined = s1.combine(s2).map(t -> {
  ...
  public void testCombineStreamingWithNonStreaming() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t10 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var countOp = Fragment.of("SELECT count(*)::INTEGER FROM t10")
  ...
  public void testStreamingQueryWithSingleColumnType() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT name FROM t11 ORDER BY id")
  ...
  public void testCursorCloseIsIdempotent() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t12 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
      cursor.next();
      cursor.close();
  ...
  public void testTransactComposition() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t13 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      List<Item> result = streaming.map(Cursor::toList).run(conn);
      assertEquals(3, result.size());
      assertEquals(new Item(1, "item1"), result.get(0));
  ...
  public void testCursorExplicitClose() {
    withConnection(conn -> {
      ...
      var streaming = Fragment.of("SELECT id, name FROM t14 ORDER BY id")
          .streamingQuery(itemCodec, 100);
      var cursor = streaming.run(conn);
