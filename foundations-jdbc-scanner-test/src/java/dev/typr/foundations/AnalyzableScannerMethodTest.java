...
class AnalyzableScannerMethodTest {
    ...
    void findsMethodsWithArgs() {
        var results = AnalyzableScanner.scanDetailed("dev.typr.foundations.fixtures.methods");
        assertTrue(fieldNames(results).contains("withArgs"), "should find method with constructible args");
    }

    @Test
    void findsTemplateMethods() {
        var results = AnalyzableScanner.scanDetailed("dev.typr.foundations.fixtures.methods");
        assertTrue(fieldNames(results).contains("templateMethod"), "should find Template-returning method");
    }
    ...
    void constructDummyCustomClass() {
        var dummy = constructDummy(
            dev.typr.foundations.fixtures.dummyargs.DummyArgFixtures.Config.class);
        assertNotNull(dummy);
    }
    ...
    void dummyArgMethodsAllDiscovered() {
        ...
        for (var expected : List.of(
            "withEnum", "withRecord", "withLocalDate", "withInstant",
            "withOffsetDateTime", "withUUID", "withList", "withOptional",
            "withMap", "withBigDecimal", "withPrimitives", "withArray",
            "withCustomClass")) {
    ...
    void kotlinStyleGetterDeduped() {
        var results = AnalyzableScanner.scanDetailed("dev.typr.foundations.fixtures.dedup");
        assertTrue(results.stream().noneMatch(r -> r.fieldName().equals("getUserQuery")),
            "getUserQuery() should be deduped as getter for userQuery field");
        assertTrue(results.stream().anyMatch(r -> r.fieldName().equals("userQuery")),
    ...
    void skipDirective() {
        ...
        var results = AnalyzableScanner.scanDetailed(
    ...
    void instanceAddsExternalObject() {
        ...
        var results = AnalyzableScanner.scanDetailed(
            ...
            ScanDirective.instance(instanceFixtures, (inst, cfg) ->
    ...
    void instanceWithSkipOverride() {
        ...
        assertTrue(results.stream().noneMatch(
                r -> r.className().equals("InstanceFixtures") && r.fieldName().equals("instanceMethod")),
    ...
    void instanceWithManualOverride() {
        ...
        var results = AnalyzableScanner.scanDetailed(
            ...
            ScanDirective.skip(dirFixtures::needsManual),
            ScanDirective.instance(instanceFixtures, (inst, cfg) ->
                cfg.manual(inst::uninvokableMethod, "manual", (Runnable) () -> {})));
        assertTrue(results.stream().anyMatch(
                r -> r.className().equals("InstanceFixtures") && r.fieldName().equals("uninvokableMethod[manual]")),
    ...
    void queryCheckerCreate() {
        var t = new Transactor(() -> { throw new UnsupportedOperationException(); }, Transactor.defaultStrategy());
    ...
    void skipExtractsMethodIdentity() {
        ...
        assertEquals("dev.typr.foundations.fixtures.directives.DirectiveFixtures", skip.declaringClass());
    ...
    void manualExtractsMethodIdentity() {
        ...
        assertEquals("dev.typr.foundations.fixtures.directives.DirectiveFixtures", manual.declaringClass());
    ...
    private static void assertNoDuplicateFieldNames(List<AnalyzableScanner.Result> results) {
        var byClass = results.stream().collect(Collectors.groupingBy(AnalyzableScanner.Result::className));
        for (var entry : byClass.entrySet()) {
            ...
            assertEquals(unique.size(), names.size(),
                "Duplicate fieldNames in " + entry.getKey() + ": " + names);
        }
    }
}
