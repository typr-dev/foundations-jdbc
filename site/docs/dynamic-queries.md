...
SQL that varies at runtime — search forms, optional filters, sort direction toggles — has two natural patterns in foundations-jdbc. They look similar; they interact with [Query Analysis](./query-analysis) very differently.
...
For most filter-style queries, prefer `optionally` — it's the only style where Query Analysis can prove every code path is well-typed against the schema.
...
This style is more flexible — you can build the list however you like (filter, flatMap, recursion, configuration-driven) — but the structure is opaque to Query Analysis.
...
- **`optionally` DSL** produces a fragment tree with explicit `Branch` nodes. The analyzer expands every branch combinatorially: 8 variants for 3 `optionally` calls, all checked.
- **`whereAnd(list)`** produces a flat concatenation. The analyzer sees one variant — whatever SQL was constructed when the test instantiated the operation.
...
- You have a small, statically-known set of optional filters (most search/list endpoints fall here).
...
- The number or shape of filters is genuinely dynamic (driven by configuration, user-defined columns, recursive structure).
...
In a typical CRUD/search application, you'll reach for `optionally` 90% of the time and `whereAnd` for the special cases.
