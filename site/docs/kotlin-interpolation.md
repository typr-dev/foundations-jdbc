...
`Sql { }` provides type-safe string interpolation for building SQL fragments in Kotlin. It lets you write SQL with Kotlin's native `${}` syntax while ensuring every value is bound as a prepared statement parameter — never concatenated into the SQL string.
