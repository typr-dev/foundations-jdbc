...
public interface PgJson<A> extends DbJson<A> {
  ...
  PgJson<dev.typr.foundations.data.Record> record =
      new PgJson<>() {
        @Override
        public JsonValue toJson(dev.typr.foundations.data.Record value) {
          return new JsonValue.JString(value.value());
        }

        @Override
        public dev.typr.foundations.data.Record fromJson(JsonValue json) {
          if (json instanceof JsonValue.JString(String value)) {
            return new dev.typr.foundations.data.Record(value);
          }
          if (json instanceof JsonValue.JObject obj) {
            ...
            sb.append(")");
            return new dev.typr.foundations.data.Record(sb.toString());
          }
          throw new IllegalArgumentException(
