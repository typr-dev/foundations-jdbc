...
public sealed interface ScanDirective {
  ...
  record ManualMethod(
      String declaringClass, String methodName, String variantName, Analyzable analyzable)
      implements ScanDirective {}

  record InstanceScan(Object instance, List<ScanDirective> overrides) implements ScanDirective {}

  @FunctionalInterface
  interface Ref0<R> extends Serializable {
    R apply();
  }

  @FunctionalInterface
  interface Ref1<A, R> extends Serializable {
    R apply(A a);
  }

  @FunctionalInterface
  interface Ref2<A, B, R> extends Serializable {
    R apply(A a, B b);
  }

  @FunctionalInterface
  interface Ref3<A, B, C, R> extends Serializable {
    R apply(A a, B b, C c);
  }
  ...
  static <A, B> ScanDirective skip(Ref2<A, B, ?> ref) {
    return skipFromLambda(ref);
  }

  static <A, B, C> ScanDirective skip(Ref3<A, B, C, ?> ref) {
    return skipFromLambda(ref);
  }
  ...
  static ScanDirective manual(
      Class<?> clazz, String methodName, String variantName, Analyzable result) {
  ...
  final class InstanceConfig<T> {
    ...
    public void skip(Ref0<?> ref) {
      directives.add(skipFromLambda(ref));
    }

    public <A> void skip(Ref1<A, ?> ref) {
      directives.add(skipFromLambda(ref));
    }

    public <A, B> void skip(Ref2<A, B, ?> ref) {
      directives.add(skipFromLambda(ref));
    }

    public <A, B, C> void skip(Ref3<A, B, C, ?> ref) {
      directives.add(skipFromLambda(ref));
    }
  ...
  private static MethodIdentity extractMethodIdentity(Serializable ref) {
    try {
      ...
      throw new IllegalArgumentException(
          "Cannot extract method identity from "
              + ref.getClass().getName()
              + ". Ensure you pass a method reference (e.g., obj::method), not a lambda.",
          e);
    }
  }
}
