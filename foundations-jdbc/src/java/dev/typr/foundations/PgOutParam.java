...
/**
 * OUT/INOUT parameter codec for PostgreSQL types.
 * Handles both JDBC type registration and value reading from CallableStatement.
 */
public interface PgOutParam<A> extends DbOutParam<A> {
  ...
  // Date/time types
  PgOutParam<LocalDate> readLocalDate = of(Types.DATE, (stmt, i) -> {
    var date = stmt.getDate(i);
    return date == null ? null : date.toLocalDate();
  });

  PgOutParam<LocalTime> readLocalTime = of(Types.TIME, (stmt, i) -> {
    var time = stmt.getTime(i);
    return time == null ? null : time.toLocalTime();
  });

  PgOutParam<LocalDateTime> readLocalDateTime = of(Types.TIMESTAMP, (stmt, i) -> {
    var ts = stmt.getTimestamp(i);
    return ts == null ? null : ts.toLocalDateTime();
  });

  PgOutParam<Instant> readInstant = of(Types.TIMESTAMP, (stmt, i) -> {
    var ts = stmt.getTimestamp(i);
    return ts == null ? null : ts.toInstant();
  });

  PgOutParam<OffsetTime> readOffsetTime = of(2013, (stmt, i) ->
      stmt.getObject(i, OffsetTime.class));

  // UUID
  PgOutParam<UUID> readUUID = of(Types.OTHER, (stmt, i) -> {
  ...
  PgOutParam<String> readObjectAsString = of(Types.OTHER, (stmt, i) -> {
    var obj = stmt.getObject(i);
    return obj == null ? null : obj.toString();
  });

  // Unboxed primitive array readers
  PgOutParam<boolean[]> readBooleanArrayUnboxed = of(Types.ARRAY, (stmt, i) -> {
  ...
  PgOutParam<short[]> readShortArrayUnboxed = of(Types.ARRAY, (stmt, i) -> {
    java.sql.Array arr = stmt.getArray(i);
    if (arr == null) return null;
    Object[] objects = (Object[]) arr.getArray();
  ...
  PgOutParam<int[]> readIntArrayUnboxed = of(Types.ARRAY, (stmt, i) -> {
    java.sql.Array arr = stmt.getArray(i);
    if (arr == null) return null;
    Object[] objects = (Object[]) arr.getArray();
  ...
  PgOutParam<long[]> readLongArrayUnboxed = of(Types.ARRAY, (stmt, i) -> {
    java.sql.Array arr = stmt.getArray(i);
    if (arr == null) return null;
    Object[] objects = (Object[]) arr.getArray();
  ...
  PgOutParam<float[]> readFloatArrayUnboxed = of(Types.ARRAY, (stmt, i) -> {
    java.sql.Array arr = stmt.getArray(i);
    if (arr == null) return null;
    Object[] objects = (Object[]) arr.getArray();
  ...
  PgOutParam<double[]> readDoubleArrayUnboxed = of(Types.ARRAY, (stmt, i) -> {
    java.sql.Array arr = stmt.getArray(i);
    if (arr == null) return null;
    Object[] objects = (Object[]) arr.getArray();
    double[] result = new double[objects.length];
    for (int j = 0; j < objects.length; j++) result[j] = ((Number) objects[j]).doubleValue();
    return result;
  });

  @SuppressWarnings("unchecked")
  PgOutParam<java.util.Map<String, String>> readMapStringString = of(Types.OTHER, (stmt, i) ->
      (java.util.Map<String, String>) stmt.getObject(i));
   ...
   */
  static <T> PgOutParam<T[]> parsedArray(java.util.function.IntFunction<T[]> arrayFactory, SqlFunction<String, T> parseElement) {
    return of(Types.ARRAY, (stmt, i) -> {
      var arr = stmt.getArray(i);
      if (arr == null) return null;
      Object[] objects = (Object[]) arr.getArray();
      T[] result = arrayFactory.apply(objects.length);
      for (int j = 0; j < objects.length; j++) {
        if (objects[j] == null) { result[j] = null; continue; }
        String text;
