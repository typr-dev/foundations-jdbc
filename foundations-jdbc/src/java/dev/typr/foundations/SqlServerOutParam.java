...
/**
 * OUT/INOUT parameter codecs for SQL Server types.
 * Each instance handles both registration and reading for its JDBC type.
 */
public interface SqlServerOutParam<A> extends DbOutParam<A> {
  ...
  SqlServerOutParam<BigDecimal> readBigDecimal = of(Types.DECIMAL, CallableStatement::getBigDecimal);
  ...
  SqlServerOutParam<LocalDate> readLocalDate = of(Types.DATE, (stmt, i) -> {
    var date = stmt.getDate(i);
    return date == null ? null : date.toLocalDate();
  });

  SqlServerOutParam<LocalTime> readLocalTime = of(Types.TIME, (stmt, i) -> {
    var time = stmt.getTime(i);
    return time == null ? null : time.toLocalTime();
  });

  SqlServerOutParam<LocalDateTime> readLocalDateTime = of(Types.TIMESTAMP, (stmt, i) -> {
    var ts = stmt.getTimestamp(i);
    return ts == null ? null : ts.toLocalDateTime();
  });

  SqlServerOutParam<Instant> readInstant = of(Types.TIMESTAMP, (stmt, i) -> {
    var ts = stmt.getTimestamp(i);
    return ts == null ? null : ts.toInstant();
  });

  SqlServerOutParam<OffsetDateTime> readOffsetDateTime = new SqlServerOutParam<>() {
    @Override public void register(CallableStatement stmt, int index) throws SQLException {
      stmt.registerOutParameter(index, microsoft.sql.Types.DATETIMEOFFSET);
    }
    @Override public OffsetDateTime read(CallableStatement stmt, int index) throws SQLException {
      return stmt.getObject(index, OffsetDateTime.class);
    }
  };

  SqlServerOutParam<String> readXmlAsString = of(Types.SQLXML, (stmt, i) -> {
    java.sql.SQLXML sqlxml = stmt.getSQLXML(i);
    return sqlxml == null ? null : sqlxml.getString();
  });

  SqlServerOutParam<Object> readSqlVariant = new SqlServerOutParam<>() {
    @Override public void register(CallableStatement stmt, int index) throws SQLException {
      stmt.registerOutParameter(index, microsoft.sql.Types.SQL_VARIANT);
    }
    @Override public Object read(CallableStatement stmt, int index) throws SQLException {
  ...
  // and Types.OTHER triggers "Implicit conversion from geography/geometry to varbinary is not allowed".
